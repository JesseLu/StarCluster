...
class SGEPlugin(clustersetup.DefaultClusterSetup):
    ...
    def _create_sge_pe(self, name="orte", nodes=None, queue="all.q"):
        ...
        # Should not count master if master_is_exec_host is set to False.
        if not self.master_is_exec_host:
            num_processors -= self._master.num_processors
    ...
    def _inst_sge(self, node, exec_host=True):
        ...
        if node.ssh.execute(inst_sge, silent=True, only_printable=True):
            node.ssh.execute(inst_sge, silent=True, only_printable=True) # Try one more time...
